 ...
 * This version supports paranthesis and
...
int f(char a)
{
	// if a is a alphabet (variable) return input precedence 7.
...
int g(char a)
{
	// if a is a alphabet (variable) return precedence 8 on stack
...
void parse_infix_to_r_polish(char infix[], char r_polish[])
{
    ...
    if(rank != 1){
		printf("Error in rank! Check your input!\n");
		return;
	}

}

void cust_strrev(char str1[], char str2[])
{
...
void parse_infix_to_polish(char infix[], char polish[])
{
	char rev_infix[1000];
	char rev_polish[1000];
	cust_strrev(infix, rev_infix);
	strcat(rev_infix, "#");
	parse_infix_to_r_polish(rev_infix, rev_polish);
	cust_strrev(rev_polish, polish);
}
...
int main()
{
    ...
    char polish[1000];
	parse_infix_to_polish(infix_string, polish);
	puts(polish);
