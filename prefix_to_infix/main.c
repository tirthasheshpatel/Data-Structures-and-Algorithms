...
->Summary : Converting `prefix` to  `infix` using STACK
*/
#include <stdio.h>
...
// Converts a prefix strint to infix.
void prefix_to_infix(char p[])
{
    // Make a stack object and initialize the `top` as -1.
    struct stack s = {-1};
    int i=strlen(p)-1;  // used to iterate through the infix string.
    do
    {
		...
		// Now if while scanning the prefix if any operator aqppears then
        ...
        else if(p[i]=='+' || p[i]=='-' || p[i]=='*' || p[i]=='/' || p[i]=='%')  //these are some binary operator
        {
            ...
            POP(&s, n1);
            strcat(n3,n2);           // concatenating `n1` in `n3`.
            size_t sz = strlen(n3);  // storing length of `n3` in `sz` variable.
            n3[sz]=p[i];             // appending the operator in `n3`
            n3[sz+1]='\0';            // every string ends with `\0`.
            strcat(n3,n1);			//appending `n2` in `n3`.
            strcat(n3, ")");        // ending the expression with `)`
            PUSH(&s,n3);           // push this expression`n3` as `variable_name` in stack.
        }


        i--;
    }while(i>=0);           // loop terminates when `\0` is scanned from `prefix`.
...
int main()
{
    char p[100];                       //`p` to store prefix expression.
    printf("Enter the prefix : ");
    scanf("%s", p);                    // scan prefix expression from user.
	prefix_to_infix(p);
}
